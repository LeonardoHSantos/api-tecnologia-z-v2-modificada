...
def update_operations(obtect_check_operation):
    ...
    conn = connetion_db()
    cursor = conn.cursor()
    print(" ------- CONECTION OPEN DATABASE | UPDATE ------- ")
    try:
       
        active = obtect_check_operation["active"]
        padrao = obtect_check_operation["padrao"]
        resultado = obtect_check_operation["resultado"]
        status_close = obtect_check_operation["status_close"]
        name_strategy = obtect_check_operation["name_strategy"]
        expiration_alert = obtect_check_operation["expiration_alert"]
        alert_time_update = obtect_check_operation["alert_time_update"]

        comando_query = f'''
        SELECT active, direction, expiration_alert, padrao, name_strategy FROM {config_db.TABLE_OPERATIONS}
        WHERE
        active = "{active}" and padrao = "{padrao}" and name_strategy = "{name_strategy}" and expiration_alert = "{expiration_alert}"
        '''
        ...
        if len(result_query) >=1:
            operation_query = result_query[0][1]
            print(f"************************ operarion_query Database: {operation_query}")
            print(f" ####### VALIDANDO QUERY: {result_query[0][1]} | RESULTADO: {resultado} | STATUS CLOSE: {status_close}")

            if operation_query == "-":
                comando_update = f'''
                UPDATE {config_db.TABLE_OPERATIONS}
                SET status_alert = "{status_close}", resultado = "N/C-{status_close}", alert_time_update = "{alert_time_update}"
                ...
                comando_update = f'''
                UPDATE {config_db.TABLE_OPERATIONS}
                SET resultado = "{status_close}", alert_time_update = "{alert_time_update}"
                WHERE name_strategy = "{name_strategy}" and expiration_alert = "{expiration_alert}" and id >= 0
                '''
                cursor.execute(comando_update)
                conn.commit()
                ...
                comando_update = f'''
                UPDATE {config_db.TABLE_OPERATIONS}
                SET resultado = "win", alert_time_update = "{alert_time_update}"
                WHERE name_strategy = "{name_strategy}" and expiration_alert = "{expiration_alert}" and id >= 0
                '''
                cursor.execute(comando_update)
                conn.commit()
                ...
                comando_update = f'''
                UPDATE {config_db.TABLE_OPERATIONS}
                SET resultado = "loss", alert_time_update = "{alert_time_update}"
                WHERE name_strategy = "{name_strategy}" and expiration_alert = "{expiration_alert}" and id >= 0
                '''
                cursor.execute(comando_update)
                conn.commit()
